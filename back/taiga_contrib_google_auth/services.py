...
ALLOW_DOMAIN = getattr(settings, "GOOGLE_API_ALLOW_DOMAIN", None)
...
def google_login_func(request, restrict_login:list=RESTRICT_LOGIN, allow_domain:list=ALLOW_DOMAIN):
    code = request.DATA.get('code', None)
    token = request.DATA.get('token', None)

    user_info = connector.me(code)
    if (RESTRICT_LOGIN != None) and (user_info.email.split("@")[1] in restrict_login):
        raise GoogleApiError({"error_message": ("Login with this Google account is disabled.")})
    if (ALLOW_DOMAIN != None) and not(user_info.email.split("@")[1] in allow_domain):
        raise GoogleApiError({"error_message": ("Login with this Google account domain is disables")})
